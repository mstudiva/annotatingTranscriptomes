...
print "
 ...
 Usage: CDS_extractor_MS.pl [fasta file] [blastx results file]
...
# writing new sequence file, reverse-complementing if necessary, writing translations as a separate file
    ...
    my $CDS = Bio::Seq->new(-seq          => $ORF,
                     -description      => "extracted_CDS " . $result->query_name,
                     -display_id => $result->query_description,
                     -alphabet         => 'dna' );
...
#	$besthit=~s/jgi\|Nemve1\|(\d+).+/$1/;
	print {HITS} $result->query_description,"\t",$qlength,"\t",length($ORF)/3,"\t",$besthit,"\t",$bh_length,"\n";
