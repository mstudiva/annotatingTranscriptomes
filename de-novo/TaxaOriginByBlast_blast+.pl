...
# modified for parallelization by C Kenkel, carly.kenkel@gmail.com
...
if ($#ARGV != 7 || $ARGV[0] eq "-h")
	...
	print "\tblastfile:\t\tname of already completed blast.br file\n";
	exit;
	}
...
# run BLAST search
	...
	if ($outopt =~ /y/)
		...
		system("legacy_blast.pl blastall -d $db -p blastx -a 16 -e $crit -v $hno -b $hno -i $qfile -o /data/$string/blast.br --path /opt/apps/blast/2.2.28/bin");
		system("cp /data/$string/blast.br .");
		system("rm -rf /data/$string");
		}
	elsif ($outopt =~ /n/)
		{
		system("legacy_blast.pl blastall -d $db -p blastx -a 16 -e $crit -v $hno -b $hno -i $qfile -o blast.br --path /opt/apps/blast/2.2.28/bin");
		}
...
#print STDERR `system("date")`;
#print "query\tmatch\tspecies\ttaxonomic_level_$ranklev\n";
$tabfile = $blastfile."."."tab";
open(TAB, ">$tabfile");
#print TAB "query", "\t", "match", "\t", "species", "\t", "taxonomic_level_", $ranklev, "\n";

my %th;
@allta = qw{kingdom phylum class family order genus species};
foreach $t (@allta) {$jcount++; $ath{$t}=$jcount; $oth{$jcount}=$t;}

my $report = new Bio::SearchIO (-file=> $blastfile, -format=>'blast');
...
# identify species from accession
		system("blastdbcmd -db $db -entry $acc -outfmt %T > $blastfile.log");	
		$taxonid = `head -n 1 $blastfile.log`; chomp($taxonid);
		system("rm $blastfile.log");
#		print $taxonid, "\n";

# extract taxonomy for that species
		my $idx_dir = "$pathi/bin";
		my $new_dir = $blastfile."."."DBdir";
		unless (-e $new_dir)
			{
